...
func (o *ReportUpdate) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(reportUpdateAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(reportUpdatePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert report_update, could not build conflict column list")
			}

			conflict = make([]string, len(reportUpdatePrimaryKeyColumns))
			copy(conflict, reportUpdatePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"report_update\"", updateOnConflict, ret, update, conflict, insert, opts...)
