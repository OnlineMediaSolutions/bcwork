...
func testAuthsExists(t *testing.T) {
	...
	e, err := AuthExists(ctx, tx, o.AuthUserID)
	if err != nil {
...
func testAuthsFind(t *testing.T) {
	...
	authFound, err := FindAuth(ctx, tx, o.AuthUserID)
	if err != nil {
		t.Error(err)
	}
...
func testAuthToOneUserUsingImpersonateAs(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	seed := randomize.NewSeed()
	if err := randomize.Struct(seed, &local, authDBTypes, true, authColumnsWithDefault...); err != nil {
		t.Errorf("Unable to randomize Auth struct: %s", err)
	}
	...
	queries.Assign(&local.ImpersonateAsID, foreign.UserID)
	if err := local.Insert(ctx, tx, boil.Infer()); err != nil {
		t.Fatal(err)
	}

	check, err := local.ImpersonateAs().One(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	if err = local.L.LoadImpersonateAs(ctx, tx, false, (*[]*Auth)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ImpersonateAs == nil {
		t.Error("struct should have been eager loaded")
	}

	local.R.ImpersonateAs = nil
	if err = local.L.LoadImpersonateAs(ctx, tx, true, &local, nil); err != nil {
		t.Fatal(err)
	}
	if local.R.ImpersonateAs == nil {
		t.Error("struct should have been eager loaded")
...
func testAuthToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := AuthSlice{&local}
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Auth)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
		t.Error("struct should have been eager loaded")
...
func testAuthToOneSetOpUserUsingImpersonateAs(t *testing.T) {
	var err error
	...
	for i, x := range []*User{&b, &c} {
		err = a.SetImpersonateAs(ctx, tx, i != 0, x)
		if err != nil {
			t.Fatal(err)
		}

		if a.R.ImpersonateAs != x {
			t.Error("relationship struct not set to correct value")
		}

		if x.R.ImpersonateAsAuths[0] != &a {
			t.Error("failed to append to foreign relationship struct")
		}
		if !queries.Equal(a.ImpersonateAsID, x.UserID) {
			t.Error("foreign key was wrong value", a.ImpersonateAsID)
		}

		zero := reflect.Zero(reflect.TypeOf(a.ImpersonateAsID))
		reflect.Indirect(reflect.ValueOf(&a.ImpersonateAsID)).Set(zero)
		...
		if !queries.Equal(a.ImpersonateAsID, x.UserID) {
			t.Error("foreign key was wrong value", a.ImpersonateAsID, x.UserID)
		}
	}
}

func testAuthToOneRemoveOpUserUsingImpersonateAs(t *testing.T) {
	var err error
	...
	if err = a.SetImpersonateAs(ctx, tx, true, &b); err != nil {
		t.Fatal(err)
	}

	if err = a.RemoveImpersonateAs(ctx, tx, &b); err != nil {
		t.Error("failed to remove relationship")
	}

	count, err := a.ImpersonateAs().Count(ctx, tx)
	if err != nil {
		t.Error(err)
	}
	if count != 0 {
	...
	if a.R.ImpersonateAs != nil {
		t.Error("R struct entry should be nil")
	}

	if !queries.IsValuerNil(a.ImpersonateAsID) {
		t.Error("foreign key value should be nil")
	}

	if len(b.R.ImpersonateAsAuths) != 0 {
		t.Error("failed to remove a from b's relationships")
	}
}

func testAuthToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	authDBTypes = map[string]string{`AuthUserID`: `character varying`, `UserID`: `character varying`, `ImpersonateAsID`: `character varying`, `CreatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
