...
func (o *RevenueDaily) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(revenueDailyAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(revenueDailyPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert revenue_daily, could not build conflict column list")
			}

			conflict = make([]string, len(revenueDailyPrimaryKeyColumns))
			copy(conflict, revenueDailyPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"revenue_daily\"", updateOnConflict, ret, update, conflict, insert, opts...)
