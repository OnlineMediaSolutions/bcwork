...
var (
	iiqDailyAllColumns            = []string{"time", "dpid", "request", "response", "impression", "revenue"}
	iiqDailyColumnsWithoutDefault = []string{"time", "dpid"}
	iiqDailyColumnsWithDefault    = []string{"request", "response", "impression", "revenue"}
	iiqDailyPrimaryKeyColumns     = []string{"time", "dpid"}
...
func FindIiqDaily(ctx context.Context, exec boil.ContextExecutor, time time.Time, dpid string, selectCols ...string) (*IiqDaily, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"iiq_daily\" where \"time\"=$1 AND \"dpid\"=$2", sel,
	)

	q := queries.Raw(query, time, dpid)
...
func (o *IiqDaily) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(iiqDailyAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(iiqDailyPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert iiq_daily, could not build conflict column list")
			}

			conflict = make([]string, len(iiqDailyPrimaryKeyColumns))
			copy(conflict, iiqDailyPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"iiq_daily\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *IiqDaily) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"iiq_daily\" WHERE \"time\"=$1 AND \"dpid\"=$2"
...
func (o *IiqDaily) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindIiqDaily(ctx, exec, o.Time, o.Dpid)
	if err != nil {
		return err
	}
...
func IiqDailyExists(ctx context.Context, exec boil.ContextExecutor, time time.Time, dpid string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"iiq_daily\" where \"time\"=$1 AND \"dpid\"=$2 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, time, dpid)
	}
	row := exec.QueryRowContext(ctx, sql, time, dpid)
...
func (o *IiqDaily) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return IiqDailyExists(ctx, exec, o.Time, o.Dpid)
}
