...
func Test_validateDemandPartnerAutomation(t *testing.T) {
	t.Parallel()
	...
	tests := []struct {
		...
		{
			name: "valid automation parameter",
			args: args{
				request: &dto.DemandPartner{
					Automation: true,
				},
		...
		{
			name: "valid threshold parameter",
			args: args{
				request: &dto.DemandPartner{
					Threshold: 0.001,
				},
		...
		},
		{
			...
			want: []string{
				"dp threshold must be in allowed list",
			},
		},
	}
	...
	for _, tt := range tests {
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			got := validateDemandPartnerAutomation(tt.args.request)
			assert.Equal(t, tt.want, got)
		})
	}
}
