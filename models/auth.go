...
func (authL) LoadImpersonateAs(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAuth interface{}, mods queries.Applicator) error {
	...
	if singular {
		...
		if !queries.IsNil(object.ImpersonateAsID) {
			args = append(args, object.ImpersonateAsID)
		}
	...
	Outer:
		for _, obj := range slice {
			...
			for _, a := range args {
				if queries.Equal(a, obj.ImpersonateAsID) {
					continue Outer
				}
			}

			if !queries.IsNil(obj.ImpersonateAsID) {
				args = append(args, obj.ImpersonateAsID)
			}
...
func (o *Auth) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"auth\"", updateOnConflict, ret, update, conflict, insert)
