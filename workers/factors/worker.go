...
func (w *Worker) Init(ctx context.Context, conf config.StringMap) error {
	...
	if w.Cron != "" {
		w.Sleep = time.Duration(bccron.Next(w.Cron))
	}
	...
	if !w.FilterExists {
		log.Warn().Err(err).Msg("Factors calculation is running on full system")
	}
...
func (w *Worker) Do(ctx context.Context) error {
	...
	var T1RecordsMap map[string]*FactorReport
	var T2RecordsMap map[string]*FactorReport
	var factors map[string]*Factor

	var err error

	start, middle, end := generateTimes(30)

	log.Info().Msg("fetch records from QuestDB")
	T1RecordsMap, T2RecordsMap, err = FetchFromQuest(ctx, start, middle, end)
	if err != nil {
		return err
	}
	if T1RecordsMap == nil || T2RecordsMap == nil {
		return errors.New("failed to fetch records from QuestDB")
	}
	...
	for _, T2record := range T2RecordsMap {
		// Check if the key exists on the first half as well
		if !w.CheckDomain(T2record.Domain) {
			continue
		}

		// Check if the key exists on the first half as well
		key := T2record.Key()
		_, exists := T1RecordsMap[key]
		if !exists || T1RecordsMap[key].Gp == 0 || T2record.Gp == 0 {
			continue
		}
		T1record := T1RecordsMap[key] // get previous hour record

		_, exists = factors[key]
		if !exists {
			continue
		}
		oldFactor := factors[key].Factor // get current running factor record
		var updatedFactor float64

		updatedFactor, err = w.CalculateFactor(T2record, T1record, oldFactor)
		if err != nil {
	...
	for _, r := range newFactors {
		...
		if r.NewFactor != r.OldFactor {
			...
			if err != nil {
				...
				log.Log().Msg(fmt.Sprintf("{Time:%s, T1:%s, T2:%s, T1GP:%.2f, T2GP:%.2f, T2GPP:%.2f, Publisher:%s, Domain:%s, Country:%s, Device:%s, OldFactor:%.2f, NewFactor:%.2f, ResponseStatus: %d}",
					r.Time.Format("2006-01-02T15:04:05Z"), r.T1.Format("2006-01-02T15:04:05Z"), r.T2.Format("2006-01-02T15:04:05Z"), r.T1GP, r.T2GP, r.T2GPP, r.Publisher, r.Domain, r.Country, r.Device, r.OldFactor, r.NewFactor, respStatus))
			}
		} else {
			log.Log().Msg(fmt.Sprintf("{Time:%s, T1:%s, T2:%s, T1GP:%.2f, T2GP:%.2f, T2GPP:%.2f, Publisher:%s, Domain:%s, Country:%s, Device:%s, OldFactor:%.2f, NewFactor:%.2f}",
				r.Time.Format("2006-01-02T15:04:05Z"), r.T1.Format("2006-01-02T15:04:05Z"), r.T2.Format("2006-01-02T15:04:05Z"), r.T1GP, r.T2GP, r.T2GPP, r.Publisher, r.Domain, r.Country, r.Device, r.OldFactor, r.NewFactor))
		}
