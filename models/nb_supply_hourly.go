...
var (
	nbSupplyHourlyAllColumns            = []string{"time", "publisher_id", "domain", "os", "country", "device_type", "placement_type", "size", "request_type", "payment_type", "bid_requests", "bid_responses", "sold_impressions", "publisher_impressions", "cost", "revenue", "avg_bid_price", "missed_opportunities", "demand_partner_fee", "data_impressions", "data_fee"}
	nbSupplyHourlyColumnsWithoutDefault = []string{"time", "publisher_id"}
	nbSupplyHourlyColumnsWithDefault    = []string{"domain", "os", "country", "device_type", "placement_type", "size", "request_type", "payment_type", "bid_requests", "bid_responses", "sold_impressions", "publisher_impressions", "cost", "revenue", "avg_bid_price", "missed_opportunities", "demand_partner_fee", "data_impressions", "data_fee"}
	nbSupplyHourlyPrimaryKeyColumns     = []string{"time", "publisher_id", "domain", "os", "country", "device_type", "placement_type", "size", "request_type", "payment_type"}
...
func FindNBSupplyHourly(ctx context.Context, exec boil.ContextExecutor, time time.Time, publisherID string, domain string, os string, country string, deviceType string, placementType string, size string, requestType string, paymentType string, selectCols ...string) (*NBSupplyHourly, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"nb_supply_hourly\" where \"time\"=$1 AND \"publisher_id\"=$2 AND \"domain\"=$3 AND \"os\"=$4 AND \"country\"=$5 AND \"device_type\"=$6 AND \"placement_type\"=$7 AND \"size\"=$8 AND \"request_type\"=$9 AND \"payment_type\"=$10", sel,
	)

	q := queries.Raw(query, time, publisherID, domain, os, country, deviceType, placementType, size, requestType, paymentType)
...
func (o *NBSupplyHourly) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(nbSupplyHourlyAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(nbSupplyHourlyPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert nb_supply_hourly, could not build conflict column list")
			}

			conflict = make([]string, len(nbSupplyHourlyPrimaryKeyColumns))
			copy(conflict, nbSupplyHourlyPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"nb_supply_hourly\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *NBSupplyHourly) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"nb_supply_hourly\" WHERE \"time\"=$1 AND \"publisher_id\"=$2 AND \"domain\"=$3 AND \"os\"=$4 AND \"country\"=$5 AND \"device_type\"=$6 AND \"placement_type\"=$7 AND \"size\"=$8 AND \"request_type\"=$9 AND \"payment_type\"=$10"
...
func (o *NBSupplyHourly) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindNBSupplyHourly(ctx, exec, o.Time, o.PublisherID, o.Domain, o.Os, o.Country, o.DeviceType, o.PlacementType, o.Size, o.RequestType, o.PaymentType)
	if err != nil {
		return err
	}
...
func NBSupplyHourlyExists(ctx context.Context, exec boil.ContextExecutor, time time.Time, publisherID string, domain string, os string, country string, deviceType string, placementType string, size string, requestType string, paymentType string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"nb_supply_hourly\" where \"time\"=$1 AND \"publisher_id\"=$2 AND \"domain\"=$3 AND \"os\"=$4 AND \"country\"=$5 AND \"device_type\"=$6 AND \"placement_type\"=$7 AND \"size\"=$8 AND \"request_type\"=$9 AND \"payment_type\"=$10 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, time, publisherID, domain, os, country, deviceType, placementType, size, requestType, paymentType)
	}
	row := exec.QueryRowContext(ctx, sql, time, publisherID, domain, os, country, deviceType, placementType, size, requestType, paymentType)
...
func (o *NBSupplyHourly) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return NBSupplyHourlyExists(ctx, exec, o.Time, o.PublisherID, o.Domain, o.Os, o.Country, o.DeviceType, o.PlacementType, o.Size, o.RequestType, o.PaymentType)
}
