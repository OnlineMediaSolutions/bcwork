...
func (publisherL) LoadConfiants(ctx context.Context, e boil.ContextExecutor, singular bool, maybePublisher interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`confiant`),
		qm.WhereIn(`confiant.publisher_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (publisherL) LoadDpoRules(ctx context.Context, e boil.ContextExecutor, singular bool, maybePublisher interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`dpo_rule`),
		qm.WhereIn(`dpo_rule.publisher in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (publisherL) LoadFloors(ctx context.Context, e boil.ContextExecutor, singular bool, maybePublisher interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`floor`),
		qm.WhereIn(`floor.publisher in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (publisherL) LoadPixalates(ctx context.Context, e boil.ContextExecutor, singular bool, maybePublisher interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`pixalate`),
		qm.WhereIn(`pixalate.publisher_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (publisherL) LoadPublisherDomains(ctx context.Context, e boil.ContextExecutor, singular bool, maybePublisher interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`publisher_domain.publisher_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Publisher) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"publisher\"", updateOnConflict, ret, update, conflict, insert)
