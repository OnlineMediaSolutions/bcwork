...
var UserRels = struct {
	...
	ManagerDpos: "ManagerDpos",
}

// userR is where relationships are stored.
type userR struct {
	ManagerDpos DpoSlice `boil:"ManagerDpos" json:"ManagerDpos" toml:"ManagerDpos" yaml:"ManagerDpos"`
}
...
func (r *userR) GetManagerDpos() DpoSlice {
	if r == nil {
		return nil
	}
	return r.ManagerDpos
}
...
// ManagerDpos retrieves all the dpo's Dpos with an executor via manager_id column.
func (o *User) ManagerDpos(mods ...qm.QueryMod) dpoQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"dpo\".\"manager_id\"=?", o.ID),
	)

	return Dpos(queryMods...)
}

// LoadManagerDpos allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadManagerDpos(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(
		qm.From(`dpo`),
		qm.WhereIn(`dpo.manager_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
	...
	if singular {
		object.R.ManagerDpos = resultSlice
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if queries.Equal(local.ID, foreign.ManagerID) {
				local.R.ManagerDpos = append(local.R.ManagerDpos, foreign)
				if foreign.R == nil {
					foreign.R = &dpoR{}
				}
...
// AddManagerDpos adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.ManagerDpos.
// Sets related.R.Manager appropriately.
func (o *User) AddManagerDpos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Dpo) error {
	var err error
	...
	if o.R == nil {
		...
		o.R.ManagerDpos = append(o.R.ManagerDpos, related...)
	}
...
// SetManagerDpos removes all previously related items of the
// user replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.Manager's ManagerDpos accordingly.
// Replaces o.R.ManagerDpos with related.
// Sets related.R.Manager's ManagerDpos accordingly.
func (o *User) SetManagerDpos(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Dpo) error {
	query := "update \"dpo\" set \"manager_id\" = null where \"manager_id\" = $1"
	...
	if o.R != nil {
		for _, rel := range o.R.ManagerDpos {
	...
	return o.AddManagerDpos(ctx, exec, insert, related...)
}

// RemoveManagerDpos relationships from objects passed in.
// Removes related items from R.ManagerDpos (uses pointer comparison, removal does not keep order)
// Sets related.R.Manager.
func (o *User) RemoveManagerDpos(ctx context.Context, exec boil.ContextExecutor, related ...*Dpo) error {
	if len(related) == 0 {
		return nil
	}
	...
	for _, rel := range related {
		for i, ri := range o.R.ManagerDpos {
			if rel != ri {
				continue
			}

			ln := len(o.R.ManagerDpos)
			if ln > 1 && i < ln-1 {
				o.R.ManagerDpos[i] = o.R.ManagerDpos[ln-1]
			}
			o.R.ManagerDpos = o.R.ManagerDpos[:ln-1]
			break
		}
	}
