...
type PriceFactorUpdateRequest struct {
...
// PriceFactorUpdateRespose
type PriceFactorUpdateRespose struct {
	// in: body
	Status string `json:"status"`
}

// PriceFactorPostHandler Update new bidder price factor.
// @Description Update new bidder price factor.
// @Tags metadata
// @Accept json
// @Produce json
// @Param options body PriceFactorUpdateRequest true "PriceFactor update Options"
// @Success 200 {object} PriceFactorUpdateRespose
// @Security ApiKeyAuth
// @Router /price/factor [post]
func PriceFactorPostHandler(c *fiber.Ctx) error {

	data := &PriceFactorUpdateRequest{}
	...
	return c.JSON(PriceFactorUpdateRespose{
		Status: "ok",
	})
}

func PriceFactorSetHandler(c *fiber.Ctx) error {
	...
	_, err := strconv.ParseFloat(factor, 64)
	if err != nil {
		c.SendString("failed to parse factor")
		return c.SendStatus(http.StatusBadRequest)
	}
	mod := models.MetadataQueue{
		Key:           "price:factor:" + publisher,
...
func PriceFactorGetHandler(c *fiber.Ctx) error {
...
func PriceFactorGetAllHandler(c *fiber.Ctx) error {

	query := `select metadata_queue.*
from metadata_queue,(select key,max(created_at) created_at from metadata_queue where key like '%price:factor%' group by key) last
where last.created_at=metadata_queue.created_at and last.key=metadata_queue.key order by metadata_queue.key`
	...
	if c.Query("format") == "html" {
		...
		for _, rec := range records {
			b.WriteString(p.Sprintf(rowPriceFactor, rec.Key, rec.Value, rec.CreatedAt.Format("2006-01-02 15:04"), rec.CommitedInstances))
		}
		t := fasttemplate.New(htmlPriceFactor, "{{", "}}")
