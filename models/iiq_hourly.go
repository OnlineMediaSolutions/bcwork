...
var (
	iiqHourlyAllColumns            = []string{"time", "dpid", "request", "response", "impression", "revenue"}
	iiqHourlyColumnsWithoutDefault = []string{"time", "dpid"}
	iiqHourlyColumnsWithDefault    = []string{"request", "response", "impression", "revenue"}
	iiqHourlyPrimaryKeyColumns     = []string{"time", "dpid"}
...
func FindIiqHourly(ctx context.Context, exec boil.ContextExecutor, time time.Time, dpid string, selectCols ...string) (*IiqHourly, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"iiq_hourly\" where \"time\"=$1 AND \"dpid\"=$2", sel,
	)
...
func (o *IiqHourly) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"iiq_hourly\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *IiqHourly) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"iiq_hourly\" WHERE \"time\"=$1 AND \"dpid\"=$2"
...
func (o *IiqHourly) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindIiqHourly(ctx, exec, o.Time, o.Dpid)
	if err != nil {
		return err
	}
...
func IiqHourlyExists(ctx context.Context, exec boil.ContextExecutor, time time.Time, dpid string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"iiq_hourly\" where \"time\"=$1 AND \"dpid\"=$2 limit 1)"
...
func (o *IiqHourly) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return IiqHourlyExists(ctx, exec, o.Time, o.Dpid)
}
