...
func testIiqHourliesExists(t *testing.T) {
	...
	e, err := IiqHourlyExists(ctx, tx, o.Time, o.Dpid, o.Datacenter)
	if err != nil {
...
func testIiqHourliesFind(t *testing.T) {
	...
	iiqHourlyFound, err := FindIiqHourly(ctx, tx, o.Time, o.Dpid, o.Datacenter)
	if err != nil {
		t.Error(err)
	}
...
var (
	iiqHourlyDBTypes = map[string]string{`Time`: `timestamp without time zone`, `Dpid`: `character varying`, `Datacenter`: `character varying`, `Request`: `bigint`, `Response`: `bigint`, `Impression`: `bigint`, `Revenue`: `double precision`}
	_                = bytes.MinRead
)
