...
func (dpoL) LoadDemandPartnerDpoRules(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDpo interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`dpo_rule`),
		qm.WhereIn(`dpo_rule.demand_partner_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Dpo) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"dpo\"", updateOnConflict, ret, update, conflict, insert)
