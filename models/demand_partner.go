...
func (demandPartnerL) LoadDemandParnterPlacements(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDemandPartner interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`demand_parnter_placement.demand_partner_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *DemandPartner) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"demand_partner\"", updateOnConflict, ret, update, conflict, insert)
