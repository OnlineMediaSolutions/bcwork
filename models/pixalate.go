...
func (o *Pixalate) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(pixalateAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(pixalatePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert pixalate, could not build conflict column list")
			}

			conflict = make([]string, len(pixalatePrimaryKeyColumns))
			copy(conflict, pixalatePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"pixalate\"", updateOnConflict, ret, update, conflict, insert, opts...)
