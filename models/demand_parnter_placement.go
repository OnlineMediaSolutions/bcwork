...
func (demandParnterPlacementL) LoadDemandPartner(ctx context.Context, e boil.ContextExecutor, singular bool, maybeDemandParnterPlacement interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`demand_partner.demand_partner_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *DemandParnterPlacement) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"demand_parnter_placement\"", updateOnConflict, ret, update, conflict, insert)
