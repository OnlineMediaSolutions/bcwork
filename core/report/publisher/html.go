...
//)
//
//func DemandReportHourly(ctx context.Context, from time.Time, to time.Time, publisherIds []string, demandPartnerIds []string, full bool) (string, error) {
//
//	boil.DebugMode = true
...
//	if !full {
//		filterZeroMod = models.DemandHourlyWhere.BidResponses.GT(0)
//	}
//
...
//		}
//
//		b.WriteString(p.Sprintf(rowDemand, rec.Time.Format("2006-01-02 15:00"), rec.DemandPartnerID, rec.PublisherID, rec.Domain, rec.BidRequests, rec.BidResponses, responseRate, bidPrice, rec.Impressions, rec.SupplyImpressions, loopRatio, fillRate, rpm, demandRPM, supplyRPM, rec.DemandTotal, rec.SupplyTotal, gp, gpp))
...
//
//	if totalImp > 0 {
//		totalSupRPM = totalSupRev / float64(totalImp) * 1000
...
//		"period": "Hourly",
//		"data":   b.String(),
//		"totals": p.Sprintf(rowBoldDemand, "Total", "", "", "", totalBidRequests, totalBidResponses, float64(totalBidResponses)/float64(totalBidRequests)*100, bidPrice, totalImp, totalPubImp, totalLoopRatio, float64(totalImp)/float64(totalBidRequests)*100, totalRPM, totalDemRPM, totalSupRPM, totalDemRev, totalSupRev, gp, gpp),
//	})
//	return s, nil
//}
//
//func DemandReportDaily(ctx context.Context, from time.Time, to time.Time, publisherIds []string, demandPartnerIds []string, full bool) (string, error) {
//
//	boil.DebugMode = true
//	publisherMod := qm.And("TRUE")
//	if len(publisherIds) > 0 {
