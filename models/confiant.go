...
var ConfiantWhere = struct {
	...
	Domain:      whereHelpernull_String{field: "\"confiant\".\"domain\""},
...
var (
	...
	confiantColumnsWithoutDefault = []string{"confiant_key", "publisher_id", "created_at"}
	confiantColumnsWithDefault    = []string{"domain", "rate", "updated_at"}
	confiantPrimaryKeyColumns     = []string{"confiant_key"}
...
func (o *Confiant) SetPublisher(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Publisher) error {
	...
	values := []interface{}{related.PublisherID, o.ConfiantKey}
...
func FindConfiant(ctx context.Context, exec boil.ContextExecutor, confiantKey string, selectCols ...string) (*Confiant, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"confiant\" where \"confiant_key\"=$1", sel,
	)

	q := queries.Raw(query, confiantKey)
...
func (o *Confiant) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
	...
	if !cached {
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"confiant\"", updateOnConflict, ret, update, conflict, insert)
...
func (o *Confiant) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"confiant\" WHERE \"confiant_key\"=$1"
...
func (o *Confiant) Reload(ctx context.Context, exec boil.ContextExecutor) error {
	ret, err := FindConfiant(ctx, exec, o.ConfiantKey)
	if err != nil {
		return err
	}
...
func ConfiantExists(ctx context.Context, exec boil.ContextExecutor, confiantKey string) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"confiant\" where \"confiant_key\"=$1 limit 1)"

	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, sql)
		fmt.Fprintln(writer, confiantKey)
	}
	row := exec.QueryRowContext(ctx, sql, confiantKey)
...
func (o *Confiant) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ConfiantExists(ctx, exec, o.ConfiantKey)
}
