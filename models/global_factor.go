...
func (o *GlobalFactor) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(globalFactorAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(globalFactorPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert global_factor, could not build conflict column list")
			}

			conflict = make([]string, len(globalFactorPrimaryKeyColumns))
			copy(conflict, globalFactorPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"global_factor\"", updateOnConflict, ret, update, conflict, insert, opts...)
