...
func (o *DemandPartnerHourly) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(demandPartnerHourlyAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(demandPartnerHourlyPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert demand_partner_hourly, could not build conflict column list")
			}

			conflict = make([]string, len(demandPartnerHourlyPrimaryKeyColumns))
			copy(conflict, demandPartnerHourlyPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"demand_partner_hourly\"", updateOnConflict, ret, update, conflict, insert, opts...)
