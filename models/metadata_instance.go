...
func (o *MetadataInstance) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(metadataInstanceAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(metadataInstancePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert metadata_instance, could not build conflict column list")
			}

			conflict = make([]string, len(metadataInstancePrimaryKeyColumns))
			copy(conflict, metadataInstancePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"metadata_instance\"", updateOnConflict, ret, update, conflict, insert, opts...)
